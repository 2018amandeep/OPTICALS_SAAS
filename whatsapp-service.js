...
function removePuppeteerLocks(dir) {
  ...
  try {
    ...
    for (const file of files) {
      ...
      if (stat.isDirectory()) {
        ...
        try {
          ...
          console.log(
            `🧹 Cleared stale Puppeteer SingletonLock at: ${filePath}`,
          );
        } catch (err) {
          console.error(
            `Could not delete lock file ${filePath}: ${err.message}`,
          );
        }
      }
    }
    ...
    console.error(
      `Error reading directory ${dir} for lock files:`,
      err.message,
    );
  }
}
...
if (!MONGODB_URI) {
  console.error(
    "❌ MONGODB_URI or MONGODB_URL is not defined in environment variables!",
  );
  process.exit(1);
}
...
function initializeWhatsAppClient() {
  ...
  client.on("ready", () => {
    ...
    latestQrImage = "";
    console.log(
      "✅ WhatsApp Web Client is ready (session loaded from MongoDB RemoteAuth)!",
    );
  });
  ...
  client.on("disconnected", async (reason) => {
    ...
    if (client) {
      ...
      try {
        ...
        console.error("Error destroying client on disconnect:", destroyErr.message);
      }
    }
...
async function sendMessage(chatId, message) {
  try {
    ...
    if (err.message && err.message.includes("detached Frame")) {
      console.log("Detached frame detected — reinitializing...");
      clientStatus = "DISCONNECTED";
      latestQrText = "";
      latestQrImage = "";
      if (client) {
        ...
        try {
          ...
          console.error("Error destroying client on detached frame:", destroyErr.message);
        }
      }
      initializeWhatsAppClient();
      throw new Error("WhatsApp reconnecting, retry in 30 seconds");
    }
    throw err;
  }
}
...
console.log(
  "🔌 Connecting to MongoDB for WhatsApp session directory storage...",
);
mongoose
  .connect(MONGODB_URI)
  .then(() => {
    ...
    puppeteerOpts = {
      ...
      args: [
        ...
        "--no-first-run",
        "--no-zygote",
        "--single-process", // KEY: for Railway/low-memory envs
        "--disable-extensions",
  ...
  try {
    if (client) {
      ...
      try {
        ...
        console.warn(
          "⚠️ client.logout() failed (likely already disconnected):",
          err.message,
        );
        ...
        if (client.authStrategy) {
          try {
            ...
            console.error(
              "❌ client.authStrategy.logout() failed:",
              strategyErr.message,
            );
          }
        }
    ...
    if (dataPath) {
      ...
      if (fs.existsSync(sessionDir)) {
        try {
          ...
          console.log(
            "🧹 Explicitly deleted local session directory:",
            sessionDir,
          );
        } catch (e) {
          console.error(
            "❌ Failed to explicitly delete local session directory:",
            e.message,
          );
        }
      }
...
app.post("/api/whatsapp/broadcast", async (req, res) => {
  ...
  if (clientStatus !== "CONNECTED") {
    return res
      .status(400)
      .json({ error: "WhatsApp client is not authenticated / connected." });
  }
  ...
  if (currentCampaign.active) {
    return res
      .status(400)
      .json({ error: "Another campaign is currently running." });
  }
...
async function runBroadcastCampaign(recipients, message, shopId) {
  console.log(
    `Starting campaign to send ${recipients.length} messages for shop: ${shopId}`,
  );
  ...
  for (let i = 0; i < recipients.length; i++) {
    ...
    if (i > 0 && i % 100 === 0) {
      console.log(
        `⏳ Reached chunk limit (100). Cooling off for 5 minutes to prevent spam banning...`,
      );
      for (let t = 300; t > 0; t--) {
    ...
    try {
      if (client) {
        ...
        console.log(
          `✅ Successfully sent broadcast message to ${recipient.name} (${phone})`,
        );
      ...
      console.error(
        `❌ Failed to send message to ${recipient.name} (${phone}):`,
        err,
      );
  ...
  try {
    if (client) {
      ...
      await WhatsAppLog.create({
        ...
        console.error("Failed to save single success log:", err),
      );
    ...
    await WhatsAppLog.create({
      ...
      console.error("Failed to save single fail log:", logErr),
    );
